"""
Phase 3 Unified: Scale Intervention on 3 models with identical conditions
=========================================================================
Re-run of Phase 3 with unified conditions:
  - Dataset: MMLU 500 samples (identical across all 3 models)
  - Position: Step 0 (prompt-last)
  - Intervention 1: unit-norm removal (h -> h/||h||)
  - Intervention 2: alpha-sweep [0.25, 0.35, 0.5, 0.7, 1.0, 1.4, 2.0, 2.8, 4.0]

Motivation: Original Phase 3 used different datasets/sample sizes
  (Qwen: Hard 500, Llama/Mistral: MMLU 300). This unifies for paper rigor.

Usage:
  python run_phase3_unified.py                    # all 3 models
  python run_phase3_unified.py --model qwen       # single model
  python run_phase3_unified.py --model llama
  python run_phase3_unified.py --model mistral

NOTE: GPU required. Forward pass only (no generation).
"""
...
BASE_DIR = POT_DIR
OUTPUT_DIR = BASE_DIR / "experiments" / "43_Phase3_Unified_Scale_Intervention"
...
def run_intervention(model_key, samples):
    """Run scale intervention for a single model using pre-loaded samples."""
    ...
    print("=" * 70)
    print(f"Phase 3 Unified: Scale Intervention on {model_name}")
    print(f"Samples: {len(samples)} MMLU (identical across all models)")
    ...
    print(f"  {'Layer':>6s} {'H_pre orig':>11s} {'H_pre unit':>11s} {'Change':>8s} "
          f"{'H_post orig':>12s} {'H_post unit':>12s} {'Change':>8s} {'h_norm':>8s}")
    print("  " + "-" * 80)
    ...
    for l in range(num_layers):
        ...
        if l in key_layers:
            print(f"  {l:6d} {np.mean(pre_orig):11.4f} {np.mean(pre_unit):11.4f} {pre_change:+8.4f} "
                  ...
                  f"{np.mean(h_norms):8.2f}")

    # Alpha-sweep at representative layers
    ...
    # H_post max variation across alphas
    ...
    if worst_var < 0.001:
        ...
        print(f"  WARNING: H_post variation {worst_var:.6f} >= 0.01 at L{worst_layer}")
    ...
    if pre_unit_mean_all > 0.99:
        ...
        print(f"  MOSTLY CONFIRMED: H_pre -> ~{pre_unit_mean_all:.4f}")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Phase 3 Unified: 3 models x MMLU 500 samples")
    parser.add_argument("--model", type=str, default="all",
                        choices=["qwen", "llama", "mistral", "all"])
    args = parser.parse_args()

    set_seed()
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # Load samples ONCE (identical for all models)
    print(f"Loading MMLU {NUM_SAMPLES} samples (shared across all models)...")
    ...
    for model_key in models_to_run:
        set_seed()  # Reset seed for each model
        analysis = run_intervention(model_key, samples)
        all_analyses[model_key] = analysis

    # Cross-model comparison
    if len(all_analyses) >= 2:
        print("\n" + "=" * 70)
        print(f"Cross-Model Comparison ({len(all_analyses)} models, MMLU {NUM_SAMPLES} samples each)")
        print("=" * 70)
        ...
        for model_key, a in all_analyses.items():
            ...
            print(f"    Samples: {a['n_samples']}, Errors: {a['n_errors']}")

        # Save cross-model summary
        summary = {
            "experiment": "Phase 3 Unified Scale Intervention",
            "dataset": "MMLU",
            "num_samples": NUM_SAMPLES,
            "condition": "identical samples across all models",
            "models": {k: {
                "h_pre_unit_mean": a["h_pre_unit_mean_all_layers"],
                "h_post_worst_var": a["h_post_worst_variation"],
                "h_post_worst_layer": a["h_post_worst_layer"],
    ...
    total_elapsed = sum(a["total_time_seconds"] for a in all_analyses.values())
    print(f"\nAll done. Total time: {timedelta(seconds=int(total_elapsed))}")
