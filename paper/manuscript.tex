...
Layerwise decoded entropy---the Shannon entropy computed from intermediate hidden states projected to vocabulary space---has attracted growing attention as an internal diagnostic \cite{ali2025entropylens, ghandeharioun2024patchscopes, kossen2024seps}. However, an implementation choice with measurement consequences has not been uniformly specified: in transformer architectures employing RMSNorm, entropy computed before versus after the final normalization step may measure qualitatively different quantities---yet reporting practices vary on which projection point is used (see the focused illustrative audit in Section~\ref{sec:implications}). RMSNorm removes radial scale from hidden states, and if pre-normalization entropy is dominated by that scale, then researchers using logit-lens entropy without normalization may be reporting hidden-state magnitude rather than distributional content.
...
This distinction has two diagnostic consequences:

\begin{enumerate}
\item \textbf{Simple scale baselines provide strong diagnostic controls in the tested settings.} In all tested post-generation settings, scale-correlated baselines (logit standard deviation, hidden-state norm) from a single forward pass match or exceed single-layer $\Hpre$ for correctness discrimination. Adding $\Hpre$ to these baselines leaves little incremental utility (95\% confidence intervals (CIs) for the change in area under the receiver operating characteristic curve (delta-AUROC) include zero in all four logit\_std+$\Hpre$ comparisons; one full-feature comparison on Mistral yields a small corrected significant condition-specific positive delta). The best internal-layer signal exceeds final-output entropy by +0.09 to +0.22 AUROC, and this advantage holds under greedy decoding (Section~\ref{sec:predictive}).
...
\item \textbf{C2. Practical consequence under matched evaluation.} Under held-out and reproduced evaluation protocols, normalization point and classifier choice substantially change analytical outcomes: $\Hpre$ and $\Hpost$ yield different per-layer AUROC landscapes, scale-correlated baselines match or exceed single-layer entropy for correctness discrimination in the tested conditions, classifier choice shifts apparent AUROC by up to +0.17, and the selective-prediction top-ranked model changes when $\Hpre$ is replaced by $\Hpost$. Profile-level scale redundancy largely persists under greedy decoding. The one corrected significant incremental exception in the eight primary sampled tests is Mistral MMLU when $\Hpre$ is added to the full scale baseline (+0.016 AUROC; adjusted $p=0.032$), which we treat as small and condition-specific rather than a universal $\Hpre$ advantage (Sections~\ref{sec:predictive}, \ref{sec:decoder}).

\item \textbf{C3. Focused illustrative audit and reporting recommendations.} We source-check the 14 examples currently included in our literature-audit table and provide a six-item reporting checklist. This focused audit documents cross-paper variation in specification practices, not invalidity or a systematic review of all references---we do not claim that any specific prior conclusion is incorrect without matched re-evaluation (Sections~\ref{sec:implications}, \ref{sec:recommendations}).
\end{itemize}
...
\textbf{Reader roadmap.} Primary evidence consists of three components: scale interventions (Section~\ref{sec:what}), held-out consequence analyses (Section~\ref{sec:predictive}), and normalization/classifier sensitivity analyses (Section~\ref{sec:decoder}). Token-position ablations (Section~\ref{sec:where}), Tuned Lens faithfulness checks and correctness-control artifacts excluded from evidentiary use, protocol-matched reproductions, and the focused illustrative literature audit are robustness or contextual analyses and should be interpreted accordingly. Section~\ref{sec:related} reviews related work. Section~\ref{sec:definitions} defines $\Hpre$/$\Hpost$ and the evaluation protocol. Section~\ref{sec:discussion} discusses implications, limitations, and reporting recommendations (C3). Section~\ref{sec:conclusion} concludes.
...
The original experiments use a single NVIDIA RTX 3090 Ti (24~GB) with half-precision floating-point (FP16) inference; projected logits are upcast to FP32 before softmax and entropy computation as described in Appendix~\ref{app:protocol}. To address numerical-precision sensitivity, we additionally ran an FP32 full-forward control for the MMLU 500 Step~0 scale-intervention setting on all three models. These FP32 controls are reported as source-anchored numerical controls, not as GPU-only replications, because the runs used automatic device mapping with CPU-mapped components. Unless otherwise noted, the default pseudorandom seed is 42; exceptions are the repeated-split analysis (split seeds 0--19) and the multi-seed generation check (generation seeds 123 and 456). Four analysis protocols are used; see the master experiment table in Appendix~\ref{app:protocol} and the Protocol note for details. Our primary cross-family evaluation uses MMLU (3 models $\times$ 1000 samples), supplemented by a Qwen mathematical reasoning case study. \textbf{Qwen serves as the deep case study} (scale interventions, token-position ablations, per-layer landscape analysis, Tuned Lens faithfulness checks); \textbf{Llama and Mistral serve as cross-family confirmation} on the core findings (scale intervention, profile redundancy, classifier sensitivity). This asymmetry reflects the iterative nature of the research and is not intended as an equal-depth comparison across models:
...
\item \textbf{Qwen Hard} (case study): competition\_math Level 4--5 \cite{hendrycks2021math}, 499 valid samples after NaN filtering (271 correct, accuracy 54.3\%). This condition uses a single model on a domain-specific mathematical-reasoning task with longer generated solutions to examine whether scale-sensitivity patterns observed on MMLU extend beyond the primary multiple-choice setting; it is not an open-ended long-form generation benchmark. An earlier run on the same items produced 55.4\% accuracy (277 correct); the difference arises from stochastic decoding and FP16 execution effects across independent runs; accordingly, we treat single-run AUROC values as point estimates rather than deterministic constants.
\end{itemize}
...
\caption{Alpha-sweep scale-intervention curves. (a)~Qwen competition\_math Hard 500 case study at L4 and L16: $\Hpre$ responds to positive rescaling while $\Hpost$ is nearly flat. (b)~Cross-family MMLU 500 representative L24 curves for Llama and Mistral under the same alpha grid. Unit-norm and worst-layer numerical summaries are reported in Table~\ref{tab:crossmodel}, rather than duplicated in the figure.}
...
\begin{table}[!ht]
\centering
\caption{Numerical cross-model scale-intervention summary (MMLU 500, Step~0, seed~=~42). This table reports source-derived summary values, rounded for display; Fig.~\ref{fig:scale_intervention} visualizes representative alpha-sweep curves. ``\% orig.\ $\Hpre{>}0.99$'' reports the rounded percentage of layers where unmodified $\Hpre$ already exceeds 0.99 (ceiling effect), distinct from the unit-norm row where all models converge to ${\sim}$1.0 by construction.}
...
Max $\alpha$-sweep $\Delta\Hpost$ & 0.000428 & 0.406987 & 0.105264 \\
Worst layer & L0 & L1 & L0 \\
h\_norm at worst & 10.21 & 0.89 & 0.25 \\
\% orig.\ $\Hpre{>}0.99$ & 4\% & 84\% & 97\% \\
\bottomrule
\end{tabular}
\end{table}

Across all three model families tested, unit-normalization collapses $\Hpre$ toward maximal entropy (mean~$>$~0.9999), confirming that $\Hpre$ is scale-sensitive in these RMSNorm-based settings (Table~\ref{tab:crossmodel}). Fig.~\ref{fig:scale_intervention} shows the complementary alpha-sweep curves without repeating the unit-norm summary as a separate panel.

\begin{table*}[!t]
\centering
\caption{Summary-level FP32 full-forward control for the same MMLU 500 Step~0 scale-intervention setting. The original columns are from the submitted Phase~3 summary; the FP32 columns are from the completed FP32 full-run control. Deltas are not shown here because the values are summary-level controls, not exact raw-value identity claims.}
\label{tab:fp32_control}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l c c c c c c c}
\toprule
Model & Orig. n/err. & FP32 n/err. & Orig. $\Hpre$ unit mean & FP32 $\Hpre$ unit mean & Orig. max $\Delta\Hpost$ & FP32 max $\Delta\Hpost$ & Worst layer (orig./FP32) \\
\midrule
Qwen & 500/0 & 500/0 & 0.999947 & 0.999947 & 0.00042795 & 0.0004236 & L0/L0 \\
Llama & 500/0 & 500/0 & 0.999989 & 0.999989 & 0.4069874 & 0.40688979 & L1/L1 \\
Mistral & 500/0 & 500/0 & 0.999999 & 0.999999 & 0.10526391 & 0.10526003 & L0/L0 \\
\bottomrule
\end{tabular}
\end{table*}

At the audited summary level, the FP32 control preserves the same completed-sample counts, $\Hpre$ unit-norm means, and worst-layer locations as the original Phase~3 summary for all three models (Table~\ref{tab:fp32_control}). The displayed $\Hpost$ worst-variation values differ only at the shown decimal scale. We use this as a numerical precision-control check for the reported summary pattern. This is not a raw-value identity claim. It does not rule out all possible precision effects.
...
In Llama and Mistral, $\Hpre$ is already near maximal entropy ($\sim$1.0) in early-to-middle layers because hidden-state norms are small (Mistral L0: 0.25 vs.\ Qwen L0: 10.21), as shown in Fig.~\ref{fig:saturation} (Appendix~\ref{app:decoder_controls}; mid-layer alpha-sweep range: Qwen L14 0.924 vs.\ Llama L16 0.008 and Mistral L16 0.0005). This ceiling effect compresses the observable alpha-sweep response, making scale sensitivity practically invisible in intermediate layers despite being structurally present.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig3.png}
\caption{Layerwise profile and saturation-limited observability. (a)~$\Hpre$ layer profiles across models: Qwen spans 0.0506--0.9968 in the plotted MMLU profile, while Llama and Mistral are near ceiling ($\sim$1.0) in early-to-middle layers. (b)~Observable $\Hpre$ alpha-sweep range for sampled alpha-sweep layers (five layers per model) versus original $\Hpre$ mean, with a log-scaled y-axis; high-$\Hpre$ ceiling layers have compressed observable ranges.}
...
\begin{enumerate}
\item \textbf{Among single-layer scalars, scale-correlated baselines outperform $\Hpre$}: logit\_std outperforms single-layer $\Hpre$ in all four sampled-generation conditions and in all three models under greedy decoding (Section~\ref{sec:greedy}). This ordering is robust at the single-split level, though the audited greedy repeated-split analysis shows weaker layer concentration in several model/metric cells, indicating that fixed-layer comparisons are sensitive to decoding protocol (Section~\ref{sec:robustness}; Appendix~\ref{app:greedy}). Multi-layer $\Hpre$ profiles outperform single-layer scale proxies, suggesting that the full entropy trajectory carries information beyond a single scalar.
...
\textbf{Sign stability (sampled generation).} $\Hpre$ sign (lower = correct) is fully stable under sampled generation: 80/80 condition-split combinations show sign~=~$-1$. The later audited greedy recomputation does not support treating greedy decoding as a broad $\Hpre$ sign-stability failure: $\Hpre$ remains 20/20 across the three greedy model conditions, while other metrics show specific exceptions such as Qwen h\_norm at 14/20 and $\Hpost$ at 19/20 on Llama and Mistral (Section~\ref{sec:greedy}, Appendix~\ref{app:greedy}). $\Hpost$ sign shows cross-family reversal in the sampled protocol: sign~=~+1 in 15/20 Llama splits vs.\ $-1$ on Qwen and Mistral (20/20 each), confirming that sign calibration on a held-out set is necessary. Additionally, h\_norm on Qwen MMLU shows sign~=~$-1$ in only 10/20 sampled splits (effectively 50/50), making it the only sampled metric-condition pair with no dominant sign direction.
...
\textbf{Task extension.} A preliminary extension to TruthfulQA mc1 \cite{lin2022truthfulqa} (817 items, Qwen; Appendix~\ref{app:truthfulqa}) shows a similar profile-level ordering (h\_norm 0.756 $>$ logit\_std 0.744 $>$ $\Hpre$ 0.742 $>$ $\Hpost$ 0.725), but $\Hpre$ incremental utility over logit\_std is a small positive (+0.016) rather than the near-zero or negative values observed on MMLU. This pattern is reported as weaker observed scale redundancy on a factual-knowledge benchmark with variable choice counts. This is a single-model, single-split observation without confidence intervals.
...
Repeated-split analysis (20 independent 70/30 splits, seeds 0--19) under sampled generation reveals that $\Hpre$ sign is fully stable: lower $\Hpre$ is associated with correct responses (sign~=~$-$) across all four conditions and all 20 splits (20/20). However, $\Hpost$ exhibits cross-family sign instability: on Llama MMLU, $\Hpost$ sign is $+$ in 15/20 splits (higher $\Hpost$ = correct), reversing the $-$ sign observed in all Qwen and Mistral conditions. This precludes a fixed-sign heuristic for $\Hpost$ and necessitates sign calibration on a held-out set. Under sampled generation, the sign reversal is specific to $\Hpost$, not $\Hpre$. Under greedy decoding, the audited recomputation does not support treating $\Hpre$ as a broad sign-stability failure; instead, sign direction is mostly stable while selected layers are less consistently concentrated in several model/metric cells (Section~\ref{sec:greedy}; Appendix~\ref{app:greedy}), so fixed single-layer heuristics should be interpreted cautiously when the decoding protocol changes.
...
On Qwen, Logit Lens achieves 0\% top-1 agreement with the final-layer distribution in middle layers (Fig.~\ref{fig:lens_dependence}a), indicating that $\Hpre$ computed via the raw lm\_head projection is a poor proxy for the final-layer prediction at those layers. Tuned Lens substantially improves faithfulness (KL(TL, final) $<$ KL(LL, final) in 27 of 28 layers; the remaining layer is the final layer, where Logit Lens is trivially exact). The same faithfulness pattern holds across all three model families (Appendix~\ref{app:tuned}). This faithfulness check does not by itself validate TL entropy as a correctness-discrimination signal in our task setting. Predictive utility and faithfulness are independent properties, and the TL correctness-control artifacts are not used as substantive evidence after the alignment audit described in Section~\ref{sec:tl_control}. The faithfulness result therefore supports a narrower conclusion: $\Hpre$ should not be interpreted as revealing what the model ``believes'' at a given layer. Out-of-sample faithfulness evaluation on a separate corpus was not performed in this study, so the faithfulness advantage aligns with prior findings but has not been independently validated here.

The scale-sensitivity distinction between $\Hpre$ and $\Hpost$ holds exactly for any linear decoder (bias = 0), including the raw logit lens. For affine decoders such as the Tuned Lens, $T_l(h)=W_lh+b_l$, scaling gives $T_l(\alpha h)=\alpha W_lh+b_l$; this is not pure temperature rescaling because of $b_l$. When $\|W_l h\| \gg \|b_l\|$, the bias is negligible and the temperature effect dominates, a regime most plausible in middle and late layers. Applying RMSNorm before $T_l$ would similarly remove radial scale. Thus, the RMSNorm confound is exact for the raw logit lens and approximate for affine decoders in large decoder-output-norm regimes.
...
\subsection{Diagnostic Consequence: Re-evaluating Entropy-Lens Under Normalization Control}
\label{sec:reeval}

The preceding sections establish that $\Hpre$ and $\Hpost$ are non-equivalent measurements with different invariance properties. Does this matter for diagnostic analysis? We test this by revisiting a concrete recent study: Entropy-Lens \cite{ali2025entropylens}, which constructs layerwise entropy profiles and uses them---via $k$-NN classification---to identify ``critical layers'' and classify model ``decision strategies'' (expansion vs.\ pruning). We conduct a matched re-evaluation using the same MMLU data (Table~\ref{tab:profile_reeval}; 3 models, 1000 samples each, generation-average), comparing $\Hpre$, $\Hpost$, and scale (logit\_std + h\_norm) profiles under both $k$-NN ($k$=3, as in the original) and logistic regression, with 5-fold stratified cross-validation.
...
\subsection{Tuned Lens Correctness-Control Audit}
\label{sec:tl_control}

As a robustness control, we initially computed Tuned Lens entropy ($\Htl$) for each layer and evaluated its correctness discrimination under a held-out protocol across the three model families. During revision, however, we audited this TL control pipeline and found an item-level alignment risk between the MMLU items used for hidden-state features and the correctness labels copied from the main experiment arrays. We therefore exclude the TL correctness-discrimination AUROC values from substantive evidence.

For each model, Tuned Lens affine translators were trained on wikitext-2 (Section~\ref{sec:faithfulness}, Appendix~\ref{app:tuned}). The training and faithfulness diagnostics remain useful as decoder-dependence context. The task-level TL AUROC artifacts, however, are retained only for transparency and are not used to support the scale-redundancy or precision conclusions.

This exclusion is not a claim that every TL value is false. It means only that the current control lacks the item-level alignment needed for final evidentiary use. Any future TL result used as substantive evidence should verify that the prompt, subject, answer label, prediction/correctness label, hidden-state features, and TL/logit-lens entropy values all refer to the same item.

\textbf{Interpretation.} We treat decoder choice as a central measurement axis and limitation, not as a minor implementation detail. The excluded TL correctness-control artifacts do not confirm or refute scale redundancy. Stronger or better-aligned decoders---longer training, task-matched data, or methods such as SimLens \cite{ma2025simlens}---remain untested (Section~\ref{sec:future}).

\subsection{Protocol-Matched Reproduction on Entropy-Lens Original Models}
\label{sec:reproduction}

As targeted corroboration, we reproduce the analysis on two Entropy-Lens original models (Llama-3-8B-Instruct and Llama-3.2-3B-Instruct) under a protocol matching five key design choices from Ali et al.\ \cite{ali2025entropylens}: 1-shot MMLU, 32-token entropy averaging, $k$-NN ($k$=3), 10-fold CV, and logit-based answer extraction (full details in Appendix~\ref{app:reproduction}). The main findings replicate: scale profiles outperform entropy profiles under LR on Llama-3-8B (0.668 vs.\ 0.617/0.620), classifier switching from $k$-NN to LR increases AUROC by +0.04 to +0.09, and the best discriminative layer shifts by 17--19 layers between $\Hpre$ and $\Hpost$. The one partial exception is Llama-3.2-3B, where $\Hpre$ LR (0.606) exceeds scale LR (0.576) by +0.030---the only model where entropy outperforms scale, occurring in a low-discrimination regime (all AUROCs 0.50--0.61). We treat these results as protocol-matched corroboration, not as expanding the paper's main claim beyond the primary three 7--8B RMSNorm models.
...
Our findings have implications for studies that compute entropy from logit-lens projections. The measurement non-equivalence documented in this paper (Sections~\ref{sec:what}, \ref{sec:reeval}) means that studies reporting ``layerwise entropy'' without specifying $\Hpre$ vs.\ $\Hpost$ may be measuring hidden-state scale, directional content, or some mixture of both. As focused illustrative context for nearby reporting practices (not a systematic review, not a source audit of all references, and not a basis for invalidating specific prior results), Table~\ref{tab:litaudit} surveys 14 cited examples and source-checks the table labels against the primary paper text or available official code where relevant.

\FloatBarrier
\begin{table*}[!t]
\centering
\caption{Focused illustrative literature audit (14 source-checked table examples, not systematic): normalization specification in prior work on layerwise entropy / internal uncertainty. Panel~A: Intermediate vocabulary projection (logit-lens family). Panel~B: Probe-based, output-level, or other internal methods.}
\label{tab:litaudit}
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.15\textwidth} >{\raggedright\arraybackslash}p{0.32\textwidth} >{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.09\textwidth}}
\toprule
...
\midrule
nostalgebraist \cite{nostalgebraist2020logitlens} & Logit Lens & GPT-2 block-output point stated; no modern RMSNorm entropy protocol & No & Text \\
Belrose et al.\ \cite{belrose2023tuned} & Tuned Lens & Yes (explicit) & No & Text + formula \\
Ali et al.\ \cite{ali2025entropylens} & Entropy-Lens & Text unspec.; code applies final norm via \texttt{ln\_final} & No & Text + code \\
Geva et al.\ \cite{geva2022transformer} & FF vocab proj. & Explicitly omitted & No & Text + appendix \\
Chuang et al.\ \cite{chuang2024dola} & DoLa & Text unspec.; code projects saved hidden states directly with \texttt{lm\_head} & No & Text + code \\
...
Chen et al.\ \cite{chen2025querylevel} & Internal Conf.\ (pre-gen.) & Projection used; norm point not specified; contextual & No & Text \\
Stolfo et al.\ \cite{stolfo2024confidence} & Conf.\ Reg.\ Neurons & Norm/scale mechanism discussed; not an entropy-profile lens & Contextual, not matched baseline & Text \\
\bottomrule
\end{tabular}
\end{table*}

Among the nine Panel~A works, normalization handling varies: Belrose et al.\ explicitly include LayerNorm, Geva et al.\ explicitly omit it and verify in an appendix, the original Logit Lens post states a GPT-2 block-output point rather than a modern pre/post-RMSNorm entropy protocol, Ali et al.\ do not specify the final normalization point in the main text but their code applies a final norm via \texttt{ln\_final}, and Chuang et al.\ leave the text unspecified while the reference code projects saved hidden states directly with \texttt{lm\_head}. The remaining four Panel~A rows do not specify the normalization point in a way that supports a modern pre/post-RMSNorm entropy distinction. No Panel~A row reports direct hidden-state norm or logit-scale baselines comparable to the controls used here. Panel~B uses probe-based, output-level, or broader internal-representation methods where the normalization point is contextual rather than a direct entropy-profile specification.

This cross-paper variation does not imply that any individual finding is invalid, but it does mean that the normalization-dependent measurement choice is not uniformly specified in this focused set. We discuss three categories of research for which the normalization point is a relevant diagnostic variable below; without matched re-evaluation under both $\Hpre$ and $\Hpost$, we do not claim that any specific prior conclusion is incorrect.
...
\textbf{Decoder design and normalization choice as separate axes.} Post-Logit-Lens methods improve decoded representations along non-equivalent axes: learned translators improve faithfulness \cite{belrose2023tuned, yomdin2024jump}, prompt-mediated frameworks broaden extraction \cite{ghandeharioun2024patchscopes, pal2023futurelens}, and task-specific decoders improve factuality or early-exit utility \cite{chuang2024dola, ma2025simlens}. However, none of these methods explicitly compares pre- and post-RMSNorm projection loci as a measurement variable. Our own Tuned Lens correctness-control AUROC artifacts are excluded from substantive evidence after an item-alignment audit (Section~\ref{sec:tl_control}). Hernandez et al. \cite{hernandez2024linearity} independently observe that layer normalization causes their linear relational embedding weights to underestimate magnitude changes, introducing a scalar correction $\beta$ to compensate---further evidence that normalization-induced scale effects are a separate concern from decoder fidelity. We therefore suggest that decoder design and normalization choice be treated as separate methodological axes.

\textbf{Known exceptions to scale redundancy.} While scale redundancy is the predominant pattern in the tested settings, four exceptions are noted: (i)~Mistral full-feature set yields a corrected significant $\Hpre$ delta of +0.016 (CI [+0.005, +0.029]; adjusted $p=0.032$ across the eight primary tests); (ii)~Llama-3.2-3B shows $\Hpre$ incremental delta of +0.024 in a low-discrimination regime; (iii)~Qwen Step~0 full-feature set shows +0.047, comparable to partition variance; (iv)~TruthfulQA mc1 shows +0.016 on a single model. Under greedy decoding, profile-level redundancy holds numerically (deltas $-$0.007 to +0.000) but statistical support drops from 3/3 to 1/3 models with CIs excluding zero. These exceptions indicate that scale redundancy is a predominant pattern in the tested settings rather than a universal law.

These implications are bounded by our experimental scope: three 7--8B RMSNorm-based models on mathematical reasoning and MMLU. Whether analogous confounds arise in LayerNorm-based architectures, larger models, or open-ended generation tasks remains an open empirical question. Concurrent work by Mar\'in \cite{marin2026rotational} independently distinguishes normalized from raw logit-lens projections and reports that applying final normalization stabilizes early-layer decoding; our work complements this by characterizing how this choice changes what decoded entropy measures across the tested model families. We frame these as diagnostic considerations rather than definitive invalidations of prior findings.
...
\item Scale baselines match or exceed single-layer entropy for correctness discrimination under both sampled and greedy protocols; fixed single-layer interpretations remain protocol-sensitive under greedy because layer selections are less consistently concentrated in several model/metric cells (Appendix~\ref{app:greedy}).
...
\begin{itemize}
\item \textbf{Model coverage and task scope}: Deep analysis (scale interventions, token positions, multi-seed check) is primarily on Qwen2.5-7B; Llama and Mistral serve as cross-family confirmation on MMLU, not full replications. A preliminary TruthfulQA extension (Appendix~\ref{app:truthfulqa}) reports slightly weaker observed scale redundancy on factual-knowledge tasks. Whether findings extend to open-ended generation, larger models, or LayerNorm-based architectures remains open.

\item \textbf{Robustness and protocol sensitivity}: Main results use a single 70/30 split (seed~=~42). Repeated-split analysis (20 splits; Appendix~\ref{app:repeated}) confirms $\Hpre$ sign stability under sampled generation (20/20) but shows layer variability. Under greedy decoding, the audited recomputation shows mostly stable sign direction but weaker layer concentration in several model/metric cells (Appendix~\ref{app:greedy}); profile-level results should be preferred over fixed single-layer heuristics. Multi-seed checks preserve profile-level ordering but show run-to-run AUROC variation.

\item \textbf{Decoder faithfulness and precision provenance}: Both $\Hpre$ and $\Hpost$ apply the final-layer decoder to intermediate representations unfaithfully in middle layers (Section~\ref{sec:faithfulness}). Entropy values should not be interpreted as reflecting the model's latent prediction. The FP32 full-forward control in Table~\ref{tab:fp32_control} checks the summary-level low-norm early-layer pattern, but it used automatic device mapping with CPU-mapped components and does not license exact raw-value identity or GPU-only claims.
...
\item \textbf{Literature audit methodology}: Table~\ref{tab:litaudit} is a focused, source-checked audit of 14 cited examples, not a systematic review or a source audit of all 39 bibliography entries; selection and coding judgments are involved, and the sample is not exhaustive.
...
\item Evaluation on open-ended generation, code generation, and long-form QA, including settings where scale may carry less task-relevant information (e.g., out-of-distribution or hallucination detection).
\item Investigation of more faithful decoders (e.g., SimLens \cite{ma2025simlens}, non-linear or task-specific decoders) under matched classifiers. The current TL correctness-control artifacts are excluded from evidentiary use, so alternative decoders remain untested as substantive controls.
\end{itemize}
...
The headline numerical values used to support the primary claims are reported in the manuscript tables and figures; the repository provides item-level manifests, extended per-layer values, and reproduction scripts for auditability. All code, Tuned Lens parameters, result files, manifests, and reproduction scripts are available at \url{https://github.com/sungmoon2/Normalization-Changes-LLM-Entropy} (commit: \texttt{5b1fd56}). Datasets (MMLU, competition\_math, TruthfulQA) and model weights (Qwen2.5-7B-Instruct, Llama-3-8B-Instruct, Mistral-7B-Instruct-v0.3) are publicly available through HuggingFace under their respective licenses.
...
Under alpha-sweep ($\alpha$ 0.25--4.0), mid-layer $\Hpre$ dynamic range is wide for Qwen (L14: 0.058--0.982, range 0.924) but at ceiling for Llama (L16: 0.992--1.000, range 0.008) and Mistral (L16: 0.9995--1.0000, range 0.0005). Full per-layer alpha-sweep tables for Llama and Mistral are provided in the repository; the paper reports only the summarized cross-model pattern (Table~\ref{tab:crossmodel}). The independent-run Tuned Lens correctness-control artifacts are provided in the repository for transparency but are excluded from substantive interpretation in this revision.
...
\setlength{\tabcolsep}{4pt}
\setlength{\tabcolsep}{1pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.06\textwidth} >{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.14\textwidth} c >{\raggedright\arraybackslash}p{0.15\textwidth} >{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.12\textwidth} >{\raggedright\arraybackslash}p{0.13\textwidth}}
\toprule
...
7.5 & TL control audit (Step 0; excluded) & MMLU (3 models) & 1000 & Step 0 & 70/30 cal/test & On cal.\ set & Excluded AUROC artifact \\
7.5 & TL control audit (gen-avg; excluded) & MMLU (3 models) & 1000 & Generation average & 70/30 cal/test & On cal.\ set & Excluded AUROC artifact \\
...
\textbf{Tuned Lens training (Section~\ref{sec:faithfulness}, Appendix~\ref{app:tuned}):} Per-layer affine translator, wikitext-2 validation, 3 epochs, Adam optimizer, lr=$10^{-3}$, KL divergence loss, final loss \mbox{7009\,$\to$\,2459}. The downstream TL correctness-control AUROC artifacts are retained for transparency only and are excluded from evidentiary use after the item-alignment audit described in Section~\ref{sec:tl_control}.
...
\textbf{Layer/sign reselection.} Each protocol or condition uses its own calibration split for best layer and sign; sampled selections are not reused.
}% end sloppy
...
h\_norm & 0.703 & .025 & L13 (17) & $+$(20) & .653--.748 \\
\bottomrule
\end{tabular}
\end{table}
...
Under greedy decoding, the audited recomputation shows mostly stable sign direction but weaker layer concentration in several model/metric cells. Greedy repeated-split and fold-wise profile comparisons are provided in the repository.
...
\textbf{TruthfulQA mc1 task extension.} A preliminary TruthfulQA mc1 extension (Qwen, 817 items, single-split) is provided in the repository. Scale redundancy patterns are broadly consistent with the MMLU findings, with slightly higher $\Hpre$ incremental utility (+0.016). TruthfulQA mc1 has variable choice counts (2--13), whereas MMLU uses four choices.
